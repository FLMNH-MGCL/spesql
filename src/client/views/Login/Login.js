...
function Login(props) {
  ...
  useEffect(() => {
    async function checkForConfig() {
      await axios.post("/api/config-check").catch((error) => {
        ...
        if (status === 418) {
          errorMessage =
            "Config folder exists, but no config JSON found. Please go to settings and correct this.";
        } else if (status === 424) {
          errorMessage =
            "Config folder created, please go to settings and set up the config JSON file.";
        } else {
          errorMessage =
            "Config folder not found and was unable to be created. Please double check the permissions of your home folder and restart the app.";
        }
