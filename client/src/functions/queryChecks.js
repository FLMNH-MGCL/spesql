    ...
    if (correctHeaders.length === headers.length) {
        ...
        correctHeaders.forEach((header, index) => {
            if (index < headers.length) {
                ...
                errors.push(
                    `INVALID COLUMN HEADER: Expected ${header}, Recieved ''`
                );
            }
        });
    } else if (headers.length > correctHeaders.length) {
        ...
        headers.forEach((header, index) => {
            if (index < correctHeaders.length) {
                if (
                    header.toLowerCase() !== correctHeaders[index].toLowerCase()
                ) {
                    errors.push(
                        `INVALID COLUMN HEADER: Expected ${correctHeaders[index]}, Recieved ${header}`
                    );
            ...
            for (let i = 1; i < arr.length; i++) {
                arr[i].split("").forEach((letter, index) => {
                    if (
                        index === 0 &&
                        upperFirst &&
                        letter.toUpperCase() !== arr[i][0]
                    ) {
                        errors.push(
                            `Format error (@ ${fieldName}): ${capsMultiple}`
                        );
                    } else if (
                        index === 0 &&
                        !upperFirst &&
    ...
    switch (fieldName) {
        ...
        case "identifiedBy":
            ...
            if (parsedName.length !== 2) {
                errors.push(
                    `Format error (@ ${fieldName}): Invalid formatting.`
                );
            } else if (parsedName[0] !== "" && parsedName[1] !== "") {
                // check last name
                if (parsedName[0][0] !== parsedName[0][0].toUpperCase()) {
                    errors.push(
                        `Format error (@ ${fieldName}): Capitalize last name.`
                    );
                }
                if (parsedName[1][0] !== parsedName[1][0].toUpperCase()) {
                    errors.push(
                        `Format error (@ ${fieldName}): Capitalize first name.`
                    );
                }
            } else {
                errors.push(
                    `Format error (@ ${fieldName}): Must provide both names.`
                );
            }
            return errors;
                ...
                if (fieldValue.indexOf("INVALID") > -1) {
                    ...
                    switch (invalidError) {
                        case "No unit specified.":
                            errors.push(
                                `Format error (@ ${fieldName}): must specify unit.`
                            );
                        default:
                            errors.push(
                                `Format error (@ ${fieldName}): invalid format.`
                            );
                    }
                    // errors.push(``)
                }
        ...
        case "freezer":
            ...
            if (!fieldValue.startsWith("Kawahara")) {
                errors.push(
                    `Format error (@ ${fieldName}): Must start with Kawahara`
                );
            } else {
                ...
                if (digs === "") {
                    errors.push(
                        `Format error (@ ${fieldName}): Missing freezer number.`
                    );
                } else {
                    // let digs = parts[1]
                    if (!isNumeric(digs)) {
        ...
        case "rack":
            ...
            if (includesPunctuation(fieldValue)) {
                errors.push(
                    `Format error (@ ${fieldName}): Remove punctuation.`
                );
            }
        ...
        case "box":
            ...
            if (!isNumeric(fieldValue)) {
                ...
                if (value < 1 || value > 20) {
                    errors.push(
                        `Number error (@ ${fieldName}): Must be from 1-20 only.`
                    );
                }
            }
